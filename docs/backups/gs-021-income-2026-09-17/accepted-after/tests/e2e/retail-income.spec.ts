...
test("an active outside coffee purchase credits once, follows its own sprite, and departs", async ({ page }, testInfo) => {
  ...
  state.rooms.push({ id: "room.retail.coffee", roomDefinitionId: "room.coffee_kiosk", x: 30, y: 27, orientation: 0, doorSide: null, upgradeLevel: 1, cleanliness: 100 });
  state.doors.push(
  ...
  state.retailExternalActors = [{
    ...
    linkedServiceOperationId: null, linkedEncounterId: null, lifecycle: "onsite", location: { x: 30, y: 28 },
  ...
  state.retailOperations = [{
    ...
    location: { x: 30, y: 28 }, returnLocation: null, path: [{ x: 30, y: 28 }], pathIndex: 0,
    lastMovedAtFacilityTick: state.facilityTick, purchaseEndsAtFacilityTick: state.facilityTick + 20, cancellationReason: null,
  }];
  ...
  const popupEvidencePromise = page.waitForFunction(() => {
    const scene = (document.querySelector("[data-testid='facility-canvas']") as any).__facilityGame.scene.getScene("facility-scene");
    const popup = [...scene.earningsPopupTexts.values()].find((label: any) => label.visible && label.text === "+$5") as any;
    const head = scene.getEarningsPopupPosition("retail_visitor", "retail-visitor.browser.1");
    return popup && head ? { popupBottom: popup.getBounds().bottom, headY: head.y } : null;
  }, undefined, { timeout: 35_000 });
  await page.getByRole("button", { name: "Resume facility time" }).click();
  const popupEvidence = await popupEvidencePromise;
  const popupPosition = await popupEvidence.jsonValue() as { popupBottom: number; headY: number };
  expect(popupPosition.popupBottom).toBeLessThan(popupPosition.headY);
  await page.screenshot({ path: "artifacts/screenshots/gs-021-m4b-retail-popup.png", animations: "disabled" });
  ...
  await expect(page.getByText("Coffee", { exact: true }).last()).toBeVisible();
  await page.screenshot({ path: "artifacts/screenshots/gs-021-m4b-management-retail.png", animations: "disabled" });
  await page.locator(".service-income-scroll").evaluate((element) => { element.scrollTop = element.scrollHeight; });
  const receiptBuyer = page.getByText("Maya Shopper", { exact: true });
  const receiptEconomics = page.getByText("$5.00 gross · $1.00 stock · $4.00 net", { exact: true });
  await expect(receiptBuyer).toBeInViewport();
  await expect(receiptEconomics).toBeInViewport();
  await page.screenshot({ path: "artifacts/screenshots/gs-021-m4b-management-retail-receipt.png", animations: "disabled" });
});
