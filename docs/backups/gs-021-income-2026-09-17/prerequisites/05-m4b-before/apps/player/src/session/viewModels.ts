...
export function createPrototypePlayerView(
  ...
  const serviceIncome: ServiceIncomeView = {
    ...
    catalogLines: SERVICE_INCOME_CATALOG.map((line) => {
      ...
      const requirementLabel = [...line.requiredCapabilityIds.map((capabilityId) => capabilityLabels.get(capabilityId) ?? humanizeIdentifier(capabilityId)), ...phaseRequirements]
      ...
      return {
        ...
        requirementLabel,
        available: !levelLocked && unavailableRequirements.length === 0,
    ...
    recentReceipts: [...state.serviceIncomeReceipts]
      ...
      .map((receipt) => ({
        ...
        actorLabel: receipt.actorKind === "remote" ? "Remote service" : receipt.actorKind,
  ...
  return {
    ...
    facility: {
      ...
      serviceVisitors: state.serviceOperations
        ...
        .map((operation) => ({
          instanceId: operation.id,
          actorId: operation.actorId,
          displayName: operation.displayName,
          appearance: operation.appearance,
          ...movementPresentation(operation.path, operation.pathIndex),
