...
function advanceGlp1Automation(state: GameState, context: DomainContext): void {
  ...
  if (completed > 0) {
    ...
    const npIds = state.employees
      .filter((employee) => employee.staffRoleDefinitionId === "staff.glp1_np")
      .map((employee) => employee.id)
      .sort();
    for (let index = 0; index < completed; index += 1) {
      const id = `income.glp1.automation.${state.facilityTick}.${index}`;
      if (state.serviceIncomeReceipts.some((receipt) => receipt.transactionKey === id)) continue;
      state.serviceIncomeReceipts.push({
        id: `${id}.${state.nextServiceIncomeReceiptSequence++}`,
        transactionKey: id,
        ...
        routeId: null,
        actorKind: "employee",
        actorId: npIds[index % npIds.length] ?? "employee.glp1.unknown",
