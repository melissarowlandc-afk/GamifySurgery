...
export class FacilityScene extends Phaser.Scene {
  ...
  private drawEarningsPopups(nowMilliseconds: number): void {
    ...
    const actorAnchors = {
      ...
      remote: Object.fromEntries(
        Object.keys(model.remoteReceiptAnchors ?? {}).map((actorId) => [actorId, true] as const),
      ),
      visitor: Object.fromEntries(
        Object.keys(model.visitorReceiptAnchors ?? {}).map((actorId) => [actorId, true] as const),
      ),
    };
  ...
  private getEarningsPopupPosition(
    actorKind: "patient" | "employee" | "founder" | "remote" | "visitor",
    actorId: string,
  ): Readonly<{ x: number; y: number }> | undefined {
    if (actorKind === "remote" || actorKind === "visitor") {
      ...
      const location = actorKind === "remote"
        ? this.bridge.viewModel.remoteReceiptAnchors?.[actorId]
        : this.bridge.viewModel.visitorReceiptAnchors?.[actorId];
      return location
        ? {
