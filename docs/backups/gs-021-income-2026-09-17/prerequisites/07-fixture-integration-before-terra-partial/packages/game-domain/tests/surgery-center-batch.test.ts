...
function preparedState(seed: string, level: 1 | 2): GameState {
  ...
  if (level === 1) {
    state.rooms.push({ id: "room.sc.examination", roomDefinitionId: "room.examination", x: 34, y: 26, orientation: 0, doorSide: "south", upgradeLevel: 1, cleanliness: 100 });
    state.doors.push({ id: "door.sc.examination", roomId: "room.sc.examination", side: "south", offset: 1, exterior: false });
  }
  if (level === 2) {
