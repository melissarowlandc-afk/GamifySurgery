...
export const BATCH_CONTENT_VERSION = "development-batch.2026-09-12.1";
...
export function createDevelopmentFamily(args: {
  ...
  const cases = args.cases.map((spec) => {
    ...
    const profiles = spec.ageYears.flatMap((ageYears) =>
      spec.sexLabels.map((sexLabel) => ({
    ...
    const nodes = spec.nodes.map((node, index) => {
      ...
      questions.push({
        ...
        patientPresentation: spec.presentation,
      ...
      timingEntries.push(timingEntry(spec.id, nodeId, questionId, node.choices));
...
export function linkSourcesToClaims(
  ...
  return sources.map((item) => ({
    ...
    evidenceClaimIds: claims
      .filter((claim) => claim.sourceIds.includes(item.id))
      .map((claim) => claim.id),
  }));
}
...
export function claim(
  args: Omit<EvidenceClaim, keyof AuthoredClinicalRecord>,
): EvidenceClaim {
  return { ...NEEDS_REVIEW, ...args };
}

export function source(
  args: Omit<ClinicalSource, keyof AuthoredClinicalRecord | "evidenceClaimIds">,
