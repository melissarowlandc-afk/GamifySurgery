          ...
          <ul className="service-income-list">
            {serviceIncome.catalogLines.map((line) => (
              <li key={line.id} className={line.available ? "" : "is-locked"}>
                <div><strong>{line.displayName}</strong><span>{line.kind} · {line.feeLabel}{line.stockCostLabel ? ` gross · ${line.stockCostLabel} stock · ${line.contributionLabel} contribution` : ""}</span></div>
