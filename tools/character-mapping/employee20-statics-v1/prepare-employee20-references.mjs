...
const receiptPath = resolve(repo, manifest.approvalReceipt.path);
...
assert.equal(sha256(receiptPath), manifest.approvalReceipt.sha256, 'approval receipt hash drift');
assert.equal(receipt.exactOwnerQuote, 'Great, those are approved.', 'unexpected approval receipt');
...
for (const record of manifest.records) {
  ...
  references.push({
    ...
    approvalAuthority: {
      receipt: manifest.approvalReceipt.path,
      receiptSha256: manifest.approvalReceipt.sha256,
      scope: receipt.approvalScope,
      exactOwnerQuote: receipt.exactOwnerQuote,
    },
  });
}
