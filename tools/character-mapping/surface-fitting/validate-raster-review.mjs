...
for(const source of Object.values(manifest.sources)){assert.equal(sha(readFileSync(resolve(root,source.sourcePath))),source.sourceSha256,`${source.costumeId} immutable source`);assert.equal(sha(readFileSync(resolve(reviewRoot,source.file))),source.sha256,`${source.costumeId} crop`);assert.deepEqual(source.directions,['south','east','west','north']);assert.deepEqual(source.tile,{width:128,height:280});assert(source.quality>=80);}
...
for(const atlas of Object.values(manifest.atlases)){assert.equal(sha(readFileSync(resolve(reviewRoot,atlas.file))),atlas.sha256,`${atlas.file} hash`);assert(atlas.quality==='lossless'||atlas.quality>=80);assert.deepEqual(atlas.variants,['green','gray','neutral']);assert.equal(sha(readFileSync(resolve(reviewRoot,atlas.guide.file))),atlas.guide.sha256,`${atlas.guide.file} hash`);assert(atlas.guide.quality==='lossless'||atlas.guide.quality>=80);}
...
async function openFragment(name,width){
  ...
  const id=`#source-faithful-character-${name}`;
...
for(const[name,info]of Object.entries(fragmentInfo)){
  ...
  for(const expectedMode of info.modes){
    ...
    for(const state of states){
      ...
      const status=await rootLocator.locator('.scr-status').textContent();assert(status.includes(state.mode)&&status.includes(state.direction));if(state.phase)assert(status.includes(state.phase));
