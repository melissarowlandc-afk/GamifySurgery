...
const reviewRoot = path.join(repo, 'artifacts/character-movement/gs019-navy-walk/review');
const fragment = path.join(reviewRoot, 'navy-walk-review.html');
...
const wrapper = path.join(reviewRoot, '.navy-review-validation.html');
...
await page.waitForFunction(() => !document.querySelector('#gs019-navy-play').disabled);
check(await page.locator('.gs019-navy-phase').count() === 8, 'eight phase buttons');
check(await page.locator('.gs019-navy-phase:disabled').count() === 0, 'phase buttons enabled after decode');
check(await page.locator('.gs019-navy-game').count() === 3, 'three game canvases');
check(await page.locator('.gs019-navy-native').count() === 3, 'three native canvases');
const phaseCanvases = await page.locator('.gs019-navy-west-thumbnail').evaluateAll(nodes => nodes.map(node => node.toDataURL()));
check(new Set(phaseCanvases).size === 8, 'eight distinct West thumbnails');
await page.locator('#gs019-navy-phase').fill('8');
check((await page.locator('#gs019-navy-phase-output').textContent()).trim() === '08', 'phase slider reaches 08');
await page.locator('#gs019-navy-play').click();
await page.waitForTimeout(205);
check((await page.locator('#gs019-navy-phase-output').textContent()).trim() === '01', 'playback wraps 08 to 01');
await page.locator('#gs019-navy-play').click();
for (const width of [736, 320]) {
  ...
  const ratios = await page.locator('.gs019-navy-game, .gs019-navy-native').evaluateAll(nodes => nodes.every(node => { const box = node.getBoundingClientRect(); return Math.abs(box.height / box.width - 2) < .02; }));
  check(ratios, `${width}px canvas aspect ratio`);
}
await browser.close();
check(pageErrors.length === 0, 'no browser page errors');
const result = { status: errors.length ? 'FAIL' : 'PASS', errors, metrics: { fragmentBytes: Buffer.byteLength(source), encoding: pins.review.encoding, distinctWestFrames: new Set(phaseCanvases).size, pageErrors } };
