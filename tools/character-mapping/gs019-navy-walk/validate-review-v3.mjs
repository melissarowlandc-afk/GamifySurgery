...
const fragment = path.join(reviewRoot, 'navy-walk-v3-review.html');
...
const wrapper = path.join(reviewRoot, '.navy-v3-review-validation.html');
...
await page.waitForFunction(() => !document.querySelector('#gs019-navy-v3-play').disabled);
check(await page.locator('.gs019-navy-v3-phase').count() === 8, 'eight phase buttons');
check(await page.locator('.gs019-navy-v3-phase:disabled').count() === 0, 'phase buttons enabled after decode');
check(await page.locator('.gs019-navy-v3-game').count() === 3, 'three game canvases');
check(await page.locator('.gs019-navy-v3-native').count() === 3, 'three native canvases');
const phaseCanvases = await page.locator('.gs019-navy-v3-west-thumbnail').evaluateAll(nodes => nodes.map(node => node.toDataURL()));
check(new Set(phaseCanvases).size === 8, 'eight distinct West thumbnails');
const mainWestFrames = [], mainEastFrames = [];
for (let phase = 1; phase <= 8; phase++) {
  await page.locator('#gs019-navy-v3-phase').fill(String(phase));
  mainWestFrames.push(await page.locator('.gs019-navy-v3-native').nth(1).evaluate(canvas => canvas.toDataURL()));
  mainEastFrames.push(await page.locator('.gs019-navy-v3-native').nth(2).evaluate(canvas => canvas.toDataURL()));
}
...
await page.locator('#gs019-navy-v3-phase').fill('8');
check((await page.locator('#gs019-navy-v3-phase-output').textContent()).trim() === '08', 'phase slider reaches 08');
await page.locator('#gs019-navy-v3-play').click();
await page.waitForFunction(() => document.querySelector('#gs019-navy-v3-phase-output').textContent.trim() === '01');
await page.locator('#gs019-navy-v3-play').click();
check((await page.locator('#gs019-navy-v3-phase-output').textContent()).trim() === '01', 'playback wraps 08 to 01');
check(await page.locator('.gs019-navy-v3-native').nth(1).evaluate((canvas, expected) => canvas.toDataURL() === expected, mainWestFrames[0]), 'West main canvas wraps to phase 01');
check(await page.locator('.gs019-navy-v3-native').nth(2).evaluate((canvas, expected) => canvas.toDataURL() === expected, mainEastFrames[0]), 'East main canvas wraps to phase 01');
for (const width of [736, 320]) {
  ...
  const ratios = await page.locator('.gs019-navy-v3-game, .gs019-navy-v3-native').evaluateAll(nodes => nodes.every(node => { const box = node.getBoundingClientRect(); return Math.abs(box.height / box.width - 2) < .02; }));
