...
const staging = resolve(repo, 'artifacts/character-movement/reauthored-pilot/olive-first-checkpoint');
const out = resolve(repo, 'artifacts/character-movement/reauthored-pilot/two-character-preview');
...
function guideFor(width, height, geometry, registration, profile) {
  ...
  const body = profile.parts.body, bodyX = hip.x + (body.bounds.x - body.anchor.x) * body.calibration.scaleX, bodyY = hip.y + (body.bounds.y - body.anchor.y) * body.calibration.scaleY;
  context.fillStyle = 'rgba(49,205,222,.22)'; context.strokeStyle = 'rgba(49,205,222,.9)'; context.lineWidth = 1.5; context.fillRect(bodyX, bodyY, body.bounds.width * body.calibration.scaleX, body.bounds.height * body.calibration.scaleY); context.strokeRect(bodyX, bodyY, body.bounds.width * body.calibration.scaleX, body.bounds.height * body.calibration.scaleY);
  context.beginPath(); profile.identityHead.polygon.forEach(([x, y], index) => { const pointX = hip.x - profile.identityHead.sourceHip.x + (profile.targetRegistration.headOffsetX ?? 0) + x, pointY = hip.y - profile.identityHead.sourceHip.y + y; index ? context.lineTo(pointX, pointY) : context.moveTo(pointX, pointY); }); context.closePath(); context.fill(); context.stroke();
...
const definitions = [
  { runtimeId: 'patient.adult.033', id: 'reauthored-olive-jacket-pilot', slug: 'olive', original: 'Photos for Codex 2/Patients or Staff or Other Characters/exec-06e158f5-4ec0-4b07-b336-befe1f93b121.png', originalSha256: '769d93e7a29a9c2b02b8cb8efa8054ec93260cc7e85c0894c1dbd00fce0ce79d', axisX: 112, floorY: 275, sourceAxisX: 95, sourceFloorY: 311 },
  { runtimeId: 'patient.adult.032', id: 'reauthored-gray-overshirt-pilot', slug: 'gray', original: 'Photos for Codex 2/Patients or Staff or Other Characters/exec-33437146-564e-4cb1-a7e2-ad41504ea752.png', originalSha256: 'b1fc32d443e65c5dcae0237ed51cbdc6838ef37a83fce9eefb0b5a254286d670', axisX: 112, floorY: 278, sourceAxisX: 98, sourceFloorY: 312 },
];
...
for (const definition of definitions) {
  ...
  const sitCrop = { x: 1000, y: 20, width: 210, height: 340 }, sit = await originalCrop(originalPath, sitCrop), sitCleanRelative = `frames/${definition.slug}-sitSouth.png`, sitGuideRelative = `frames/${definition.slug}-sitSouth-guide.png`, sitBytes = sit.toBuffer('image/png'), sitRegistration = { axisX: 105, floorY: 312 };
  ...
  const sourceCrop = { x: 50, y: 20, width: 210, height: 340 }, sourcePreview = await originalCrop(originalPath, sourceCrop), previewRelative = `frames/${definition.slug}-source-standing.png`, previewBytes = sourcePreview.toBuffer('image/png'); writeFileSync(resolve(out, previewRelative), previewBytes);
...
const manifest = { schemaVersion: 1, status: 'two-character-pilot-review', acceptedSolvers: ['tools/character-mapping/math.mjs#compileLateral', 'tools/character-mapping/math.mjs#compileNorthSouth'], directions, phaseIds, characters, validation: { frameCount, assembledOpaqueComponentsPerFrame: 1, retainedMagentaDominantPixels: 0 }, limitations: ['Pilot preview only; visual owner acceptance remains pending.', 'Standing uses the new fitted neutral assembly; sitting remains the approved original static pose with explicit provenance.'] };
