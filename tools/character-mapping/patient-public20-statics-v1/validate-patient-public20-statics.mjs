...
const approvalReceiptPath = `${outputRoot}/approvals/patient-public20-front-concepts-user-approval-v1.json`;
const receipt = JSON.parse(readFileSync(resolve(repo, approvalReceiptPath), 'utf8'));
...
const preservation = JSON.parse(readFileSync(resolve(import.meta.dirname, 'prior-statics-preservation-914.json'), 'utf8'));
...
assert.deepEqual(manifest.records.map((record) => record.id), expectedIds, 'patient/public IDs drifted');
...
assert.equal(receipt.exactUserQuote, 'Create stills for the 20 more new characters (patients or general public) who were just approved');
assert.equal(sha(resolve(repo, receipt.sourceIntake.path)), receipt.sourceIntake.sha256, 'approval receipt intake binding changed');
const frozenReceiptHashes = new Set(references.references.map((entry) => entry.approvalAuthority.receiptSha256)); assert.equal(frozenReceiptHashes.size, 1, 'reference receipt bindings disagree'); assert.equal(sha(resolve(repo, approvalReceiptPath)), [...frozenReceiptHashes][0], 'approval receipt changed after reference acceptance');
assert.equal(receiptById.size, 20);
assert.equal(references.references.length, 20);
assert.deepEqual(preservation.counts, { gs018Baseline: 704, gs018Clipboard: 30, employee20: 160, patientPublicOriginalFronts: 20, total: 914 });
for (const entry of preservation.manifests) assert.equal(sha(resolve(repo, entry.path)), entry.sha256, `prior manifest preservation guard failed: ${entry.path}`);
for (const entry of preservation.files) assert.equal(sha(resolve(repo, entry.file)), entry.sha256, `prior static preservation guard failed: ${entry.file}`);
...
for (const record of manifest.records) {
  ...
  assert.equal(record.category, sourceRecord.category); assert.equal(record.appearanceSpec, sourceRecord.appearanceSpec); assert.equal(record.intendedAge, sourceRecord.intendedAge); assert.equal(record.intendedSex, sourceRecord.intendedSex); assert.equal(record.displayGender, sourceRecord.displayGender); assert.deepEqual(record.demographicEvidence, sourceRecord.demographicEvidence, `${record.id} demographic metadata drift`);
  const south = record.poses.stand.south; assert.equal(south.status, 'user-approved-original-front-reused'); assert.equal(south.sha256, receiptEntry.sha256); assert.equal(sha(resolve(repo, south.file)), receiptEntry.sha256, `${record.id} final South is not byte-preserved`); assert.deepEqual(readFileSync(resolve(repo, south.file)), readFileSync(resolve(repo, receiptEntry.file)), `${record.id} South bytes differ`);
