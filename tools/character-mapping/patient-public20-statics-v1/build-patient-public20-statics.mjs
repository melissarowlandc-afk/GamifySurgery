...
const assetsRoot = 'tools/character-mapping/patient-public20-statics-v1/assets';
const manifest = JSON.parse(readFileSync(resolve(repo, intakeRoot, 'manifest.json'), 'utf8'));
const approvalReceipt = { path: `${outputRoot}/approvals/patient-public20-front-concepts-user-approval-v1.json` };
const receipt = JSON.parse(readFileSync(resolve(repo, approvalReceipt.path), 'utf8'));
const references = JSON.parse(readFileSync(resolve(repo, outputRoot, 'reference-manifest.json'), 'utf8'));
const config = JSON.parse(readFileSync(resolve(import.meta.dirname, 'patient-public20-config.json'), 'utf8'));
const approvedSnapshots = new Map();
for (const id of config.artApprovedIds ?? []) {
  const number = id.slice(-3), snapshotPath = resolve(import.meta.dirname, `approved-person${number}-7.json`);
...
approvalReceipt.sha256 = references.references[0].approvalAuthority.receiptSha256;
assert(references.references.every((entry) => entry.approvalAuthority.receipt === approvalReceipt.path && entry.approvalAuthority.receiptSha256 === approvalReceipt.sha256), 'reference approval-receipt bindings disagree');
...
assert.equal(shaFile(resolve(repo, approvalReceipt.path)), approvalReceipt.sha256, 'approval receipt changed');
assert.equal(shaFile(resolve(repo, receipt.sourceIntake.path)), receipt.sourceIntake.sha256, 'approval receipt intake binding changed');
...
for (const intake of manifest.records) {
  ...
  const record = { id, category: intake.category, appearanceSpec: intake.appearanceSpec, intendedAge: intake.intendedAge, intendedSex: intake.intendedSex, displayGender: intake.displayGender, demographicEvidence: intake.demographicEvidence, ownerApproval: 'user-approved-identity-and-front-concept-only', runtimeReady: false, reference: references.references.find((entry) => entry.id === id), approvedSouth: { sourceFile: approved.file, sourceSha256: approved.sha256, receipt: approvalReceipt, outputFile: relative('characters', id, 'stand-south.png'), outputSha256: approved.sha256, copyPolicy: 'byte-for-byte approved original reuse', ...southMeasure }, generatedSource: null, poses: { stand: { south: { status: 'user-approved-original-front-reused', file: relative('characters', id, 'stand-south.png'), sha256: approved.sha256, anchors: { bodyAxisX: 80, floorY: 287 }, ...southMeasure } }, sit: {} }, proofs: null };
  const assetVersion = config.assetVersions[id] ?? 'v1', sourceOrder = config.sourceOrders[id] ?? views; assert.deepEqual([...sourceOrder].sort(), [...views].sort(), `${id} source order must contain each cardinal direction once`); const assetRelative = `${assetsRoot}/${id}-cardinals-${assetVersion}.png`, promptRelative = `${assetsRoot}/${id}-cardinals-${assetVersion}-exact-prompt.txt`; const assetPath = resolve(repo, assetRelative), promptPath = resolve(repo, promptRelative); const provenanceRelative = [`${assetsRoot}/${id}-cardinals-${assetVersion}-tool-output-provenance.txt`, `${assetsRoot}/${id}-cardinals-${assetVersion}-provenance.md`].find((candidate) => existsSync(resolve(repo, candidate))), provenancePath = provenanceRelative ? resolve(repo, provenanceRelative) : null;
  const sourcePairReady = existsSync(assetPath) && existsSync(promptPath) && Boolean(provenancePath);
  if (sourcePairReady) {
