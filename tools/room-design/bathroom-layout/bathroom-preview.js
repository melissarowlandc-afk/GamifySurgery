(()=>{
  ...
  function drawShell(){
    ...
    const tall=i=>{const sx=X+i*S;ctx.fillStyle='#c6dfd7';ctx.fillRect(sx,Y-wall,S,wall);ctx.fillStyle='rgba(146,110,67,.055)';ctx.fillRect(sx+12,Y-wall+28,S-24,2);ctx.fillStyle=green;ctx.fillRect(sx,Y-23,S,23);ctx.fillStyle=light;ctx.fillRect(sx,Y-23,S,4);ctx.fillStyle=dark;ctx.fillRect(sx,Y-7,S,7);ctx.fillRect(sx-1,Y-wall-9,S+2,9);ctx.fillStyle=light;ctx.fillRect(sx-1,Y-wall-9,S+2,3)};
