...
const COLUMNS=5,ROWS=10,MAP_W=96,MAP_H=144,THUMB_W=96,THUMB_H=112,PORTRAIT_W=192,PORTRAIT_H=224;
...
function mapFrame(image,actors,id,pose){const {crop,bounds}=frame(image,actors,id,pose),target=createCanvas(MAP_W,MAP_H),ctx=target.getContext("2d");ctx.imageSmoothingEnabled=false;const scale=Math.min(84/bounds.width,128/bounds.height),width=Math.max(1,Math.round(bounds.width*scale)),height=Math.max(1,Math.round(bounds.height*scale));if(height<58||width<26)throw new Error(`${id} ${pose}: implausibly small extracted actor.`);ctx.drawImage(crop,bounds.x,bounds.y,bounds.width,bounds.height,Math.round((MAP_W-width)/2),136-height,width,height);removeRemoteNoise(target);return target;}
/**
 ...
 */
function mirrorMapFrame(canvas){const target=createCanvas(MAP_W,MAP_H),ctx=target.getContext("2d");ctx.imageSmoothingEnabled=false;ctx.translate(MAP_W,0);ctx.scale(-1,1);ctx.drawImage(canvas,0,0);return target;}
/**
...
function directionalWalkNeutral(canvas){
  const target=createCanvas(MAP_W,MAP_H),ctx=target.getContext("2d");ctx.imageSmoothingEnabled=false;ctx.drawImage(canvas,0,0);
  const source=canvas.getContext("2d").getImageData(0,0,MAP_W,MAP_H),out=ctx.getImageData(0,0,MAP_W,MAP_H),data=out.data;
  for(let y=96;y<MAP_H;y++)for(let x=0;x<MAP_W;x++){const at=(y*MAP_W+x)*4;data[at+3]=0;}
  for(let y=96;y<MAP_H;y++){
    const inset=Math.min(7,Math.max(0,Math.round((y-94)*.15)));
    for(let x=0;x<MAP_W;x++){
      const from=(y*MAP_W+x)*4;if(source.data[from+3]<=12)continue;
      const targetX=Math.max(0,Math.min(MAP_W-1,x+(x<48?inset:x>48?-inset:0))),to=(y*MAP_W+targetX)*4;
...
function directionalOppositeStride(canonicalA,sourceB){
  const target=createCanvas(MAP_W,MAP_H),ctx=target.getContext("2d");ctx.imageSmoothingEnabled=false;ctx.drawImage(canonicalA,0,0);
  const lower=sourceB.getContext("2d").getImageData(0,96,MAP_W,MAP_H-96);
  ctx.clearRect(0,96,MAP_W,MAP_H-96);ctx.putImageData(lower,0,96);return target;
}
function uiFrame(image,actors,id,pose){const {crop,bounds}=frame(image,actors,id,pose),[width,height]=pose==="thumbnail"?[THUMB_W,THUMB_H]:[PORTRAIT_W,PORTRAIT_H],target=createCanvas(width,height),ctx=target.getContext("2d"),pad=pose==="thumbnail"?4:5;ctx.imageSmoothingEnabled=false;const scale=Math.min((width-pad*2)/bounds.width,(height-pad*2)/bounds.height),dw=Math.max(1,Math.round(bounds.width*scale)),dh=Math.max(1,Math.round(bounds.height*scale));ctx.drawImage(crop,bounds.x,bounds.y,bounds.width,bounds.height,Math.round((width-dw)/2),Math.round((height-dh)/2),dw,dh);removeRemoteNoise(target);return target;}
...
function atlas(frames,width,height){const canvas=createCanvas(width*COLUMNS,height*ROWS),ctx=canvas.getContext("2d");ctx.imageSmoothingEnabled=false;frames.forEach((item,index)=>ctx.drawImage(item,index%COLUMNS*width,Math.floor(index/COLUMNS)*height));return canvas;}
...
const manifest={contentRevision:"patients-v1-r6",sourceContract:{grid:[6,3],reference:"Photos for Codex/exec-e644a34b-c74e-4787-b593-d009b00768ac.png",strategy:"row actor detection + per-actor exterior-connected checker removal",poseSlots:sourceSlots,horizontalDirectionSlots:{right:{a:[4,1],b:[5,1]},left:{a:"mirror:right-walk-a",b:"mirror:right-walk-b"}}},horizontalWalkDerivation:{"right-walk-b":{lowerLimbRowsFrom:"right-walk-b-source",upperBodyRowsFrom:"right-walk-a",upperBodyRows:[0,95],lowerBodyRows:[96,143]},"left-walk-a":{mirrorOf:"right-walk-a"},"left-walk-neutral":{mirrorOf:"right-walk-neutral"},"left-walk-b":{mirrorOf:"right-walk-b"},"right-walk-neutral":{legInsetOf:"right-walk-a",upperBodyRows:[0,95],lowerBodyRows:[96,143]}},columns:COLUMNS,rows:ROWS,mapCell:{width:MAP_W,height:MAP_H,floorAnchor:{x:48,y:136},seatAnchor:{x:48,y:102}},thumbnailCell:{width:THUMB_W,height:THUMB_H},portraitCell:{width:PORTRAIT_W,height:PORTRAIT_H},variants:loaded.map(source=>({id:source.id,source:source.file,sourceSha256:sha(source.bytes)})),poses:poseFiles,walkPhases:{a:"left-foot-forward",neutral:"feet-together",b:"right-foot-forward"},decodedBytesEstimate:(mapPoses.length*MAP_W*MAP_H*COLUMNS*ROWS+THUMB_W*THUMB_H*COLUMNS*ROWS+PORTRAIT_W*PORTRAIT_H*COLUMNS*ROWS)*4};writeFileSync(resolve(out,"manifest.json"),JSON.stringify(manifest,null,2));
save(proof("Patient V1: map / thumbnail / portrait",["front-idle","thumbnail","portrait"],assets,.30),resolve(proofDir,"patient-v1-identities.png"));save(proof("Patient V1: seating and exam table",["seated-front","seated-left","seated-right","exam-table"],assets,.32),resolve(proofDir,"patient-v1-seating.png"));save(proof("Patient V1: direction-locked lateral A / neutral / B gait frames",["left-walk-a","left-walk-neutral","left-walk-b","right-walk-a","right-walk-neutral","right-walk-b"],assets,.25),resolve(proofDir,"patient-v1-gaits.png"));console.log(`Built ${loaded.length} clean patient identities x ${Object.keys(assets).length} families; ${(manifest.decodedBytesEstimate/1024/1024).toFixed(1)} MiB decoded RGBA.`);
